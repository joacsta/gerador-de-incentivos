...
from app.constants.constants import obter_caminho_producao, obter_caminho_rascunho
...
def gerar_script_processamento(
    ...
    if servidor.nome_servidor == AMBIENTE_TESTE:
        ...
        with open(
            diretorio_rascunho / "draft-processamento.sql", "w", encoding="utf-8"
        ) as f:
            f.write(sql_query)
    ...
    with open(
        diretorio_producao / "passo-1-processamento.sql", "w", encoding="utf-8"
    ) as f:
        f.write(sql_query)
